...
//Permite listar todos los productos guardados en el carrito.
 cart.get('/:id/products', async (req, res) => {
...
cart.post('/:id/products', async (req, res) => {
    ...
    const indexProduct = products1.findIndex((obj) => obj.id == idProduct);
...
cart.delete('/:id', async (req, res: Response) => {
	const id = req.params.id;
	const carts = JSON.parse(await getData())
